...
export default function CropPreviewModal({ imageUrl, pageNumber, answerText, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-4xl glass-panel border border-slate-700/80 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-slate-800 bg-slate-900/90 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-xs font-semibold text-emerald-400 flex items-center gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-slate-100 transition ml-2"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-auto p-6 bg-slate-950/90 flex items-center justify-center min-h-[350px]">
          <div
        ...
        {answerText && (
          <div className="px-6 py-3 border-t border-slate-800 bg-slate-900/90 text-xs text-slate-300 flex items-center justify-between">
            <span className="text-slate-400">Extracted Answer Snippet:</span>
            <span className="font-semibold text-emerald-400 truncate max-w-xl">{answerText}</span>
          </div>
        )}
