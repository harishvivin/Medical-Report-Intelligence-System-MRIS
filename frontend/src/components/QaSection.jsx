...
export default function QaSection({ documentId, onOpenCropModal }) {
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      {/* Search Input Bar */}
      <div className="glass-panel p-6 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-xl">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            handleAsk();
        ...
        >
          <div className="relative flex items-center">
            <input
              ...
              className="w-full pl-5 pr-28 py-4 rounded-xl glass-input text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500/50 border border-slate-300 dark:border-slate-700/60 transition text-sm font-medium"
            />
            <button
              type="submit"
        ...
        {error && (
          <div className="mt-4 p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-xs text-red-600 dark:text-red-400 flex items-center gap-2">
      ...
      <div className="space-y-4">
        {qaHistory.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="glass-panel p-6 rounded-2xl border border-slate-200/90 dark:border-slate-800/80 transition-all hover:border-slate-300 dark:hover:border-slate-700"
            >
              {/* Question Header */}
              <div className="flex items-start justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-3 mb-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {item.pageNumber && (
                    <span className="px-2.5 py-1 rounded-md glass-card border border-slate-200 dark:border-slate-800 text-[11px] text-slate-700 dark:text-slate-300 font-medium flex items-center gap-1">
                ...
                {item.snippetUrl && (
                  <div className="lg:col-span-5 flex flex-col items-center">
                    <div
                      ...
                      className="group relative w-full overflow-hidden rounded-xl bg-slate-900 border border-sky-500/30 cursor-pointer hover:border-sky-400 hover:shadow-glow-sky transition-all duration-300"
                    >
                      <div className="relative aspect-[4/3] w-full bg-slate-950 flex items-center justify-center">
                        ...
                        />
                        <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 text-xs font-semibold text-sky-300 backdrop-blur-xs">
